...
The first comprehensive review of the field in book form, by \cite{stoddenETal2014}, includes this passage in the preface:
...
\cite{stodden2015} does not distinguish the two recurrent terms: 
...
\section*{Expanded terminology}
\vspace{\up}

\cite{stodden2013} unpacks the concerns into \emph{empirical} reproducibility, \emph{computational} reproducibility, and \emph{statistical} reproducibility.
But a few others have also invented expanded terminologies. 
We already saw that in the fields of economics and political science, the umbrella term is \emph{replication}. 
\cite{hamermesh2007} talks about \emph{pure} replication, \emph{statistical} replication, and \emph{scientific} replication. The first refers to re-analyzing the same data with the same model and estimation parameters; the second may use different but comparable data, but the same model. 
...
These definitions are consistent with the Claerbout/Donoho/Peng convention (group $B1$), even though  in economics and political science `replicability` is widely used as an umbrella term.


The NSF Directorate for Computer \& Information Science \& Engineering (CISE) issued a ``Dear Colleague Letter: Encouraging Reproducibility in Computing and Communications Research'' in October 21, 2016.
